...
function Hero() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=USD&order=gecko_desc&per_page=${coinsPerPage}&page=${currentPage}&sparkline=false&price_change_percentage=24h`;
          ...
          {loading ? (
            ...
            cryptoData.map((crypto) => (
              <tr className='tableTr' key={crypto.id}>
                ...
                <td className={`tableItems tablePercentage ${crypto.price_change_percentage_24h < 0 ? 'negative' : ''}`}>{crypto.price_change_percentage_24h > 0 ? '+' : ''}{crypto.price_change_percentage_24h}%</td>
